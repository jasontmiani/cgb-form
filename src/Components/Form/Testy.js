...
*/
/**
 * TODO-later => Seperate into reusable components
 * inline form validation from react-hook-form
 *
 * @export SelectCountryCode()
 * @returns form && @data = {
 *     name,
 *     email,
 *     password,
...
export default function StartTrial() {
    ...
    //NOTE useForm - does a few things. creates form data and forces inline validation based on @params
  ...
  return () => {
    <form onSubmit={handleSubmit(onSubmit)}>
      <input type="text" placeholder="Name" name="name" ref={register({required: true, min: 4, maxLength: 80, pattern: /\D/i})} />
  ...
  export function SelectCountryCode(defaultCountryId, defaultCountry) {
    
    return (
      <select name="countryCode" ref={register( { required: true } )}>
      <option value={defaultCountryId}>{{ defaultCountry }}</option>
